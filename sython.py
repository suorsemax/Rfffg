...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.Halo"))
async def _(event):
...
@sython.on(events.NewMessage(outgoing=False, pattern='ماكس'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('Hello my friend ➧ @YFYYF')
...
@sython.on(events.NewMessage(outgoing=True, pattern=".say"))
async def _(event):
      await event.reply("""The source is working"""
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
